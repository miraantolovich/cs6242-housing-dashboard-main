...
// Houtan's og margin, width, & height
...
var chartMargin = {top:50, left:90, right:125, bottom:50};
var chartWidth = 2/3 * (900 - chartMargin.left - chartMargin.right);
var chartHeight = 325 - chartMargin.top - chartMargin.bottom;
...
var predMargin = {top:50, left:90, right:125, bottom:50};
var predWidth = 2/3 * (900 - predMargin.left - predMargin.right);
var predHeight = 330 - predMargin.top - predMargin.bottom;
...
function drawLineChart(data) {
    ...
    var lines = [
        { name: "Population", color: "#0cc35c", values: data.map(function(d) { return { x: +d.Year, y: +d.Population }; }) },
        { name: "Employment", color: "#0c73c3", values: data.map(function(d) { return { x: +d.Year, y: +d.Employment }; }) },
        { name: "AVG Price", color: "#cb5b04", values: data.map(function(d) { return { x: +d.Year, y: +d['AVG_Price'] }; }) }
    ];
    var line = d3.line()
...
function predResult(stateName) {
    ...
    function getData() {
        ...
        //console.log(city,", ",state); //KIM this bit works. we have the selected city + state
...
d3.json("us-states_modified.json").then( function(data) {
    mapChart.append("g")
        ...
        .on("mouseover", function(d) {
                                        if (clicked == 0) {
                                            d3.select(this).attr("fill", "#cb0474");
        ...
        .on("click", function(d) {
            ...
            if (clicked == 1) {
                d3.select(selectedState).style("fill","#cb0474");
                ...
                if (selectedState === d.properties.name) {
                    ...
                    lineChart.append("text")
                        ...
                        .attr("y", -25)
                        .text("Historic Trends for " + clickedState)
                        .style("font-size", "16px")
                    ...
                    predChart.append("text")
                        ...
                        .attr("y", -25)
                        .text("Average House Price Prediction for 2024 in " + clickedState)
                        .style("font-size", "16px")
                        .style("font-weight", "bold");
                }
            } else {
